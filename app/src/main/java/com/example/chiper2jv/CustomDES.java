...
public class CustomDES {
    ...
    public String code(String message){
        ...
        byte[] c = this.deal(getCustomBytes(message), 1);
        return new String(c, StandardCharsets.UTF_8);
    }

    public String decode(String code , String kay){

        generateKeys(kay);

       byte[] c = this.deal(getCustomBytes(message), 1);
       String s = new String(c);
       byte[] t = getCustomBytes(code);
       byte[] p = deal(c , 0);
    ...
    public static byte[] getCustomBytes(String word) {
        return word.getBytes(StandardCharsets.UTF_8);
    }



    public static byte[] getCustomBytes(byte[] bytes) {
        byte[] out = new byte[bytes.length];

        for (int i = 0; i < bytes.length; i++) {
            out[i] = (byte) (bytes[i] & 0xff);
        }

        return bytes;
    }

    private static byte[] castom(String word) {
    ...
    public byte[] deal(byte[] p, int flag) {
        ...
        if (flag == 0) {// расшифровка
            ...
            System.arraycopy(result_data, 0, p_result_data, 0,Math.min(p_origin_length , result_data.length));
    ...
    public byte[] descryUnit(byte[] p, int k[][], int flag) {
        ...
        for (int i = 0; i < 8; i++) {
            c_byte[i] = (byte) ((c[8 * i] << 7) + (c[8 * i + 1] << 6) + (c[8 * i + 2] << 5)
                    + (c[8 * i + 3] << 4) + (c[8 * i + 4] << 3) + (c[8 * i + 5] << 2) + (c[8 * i + 6] << 1) + (c[8 * i + 7]));
        }
        return c_byte;
